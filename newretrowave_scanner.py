...
# ==========================================
# CONFIGURACION DE COLORES MATRIX
# ==========================================
BG_COLOR = "#000000"          # Negro apagado (para contenedores sin transparencia native)
TEXT_NEON_GREEN = "#00FF41"   # Verde matrix brillante
TEXT_DARK_GREEN = "#008F11"   # Verde oscuro
BTN_BG = "#002200"            # Fondo dark button
BTN_FG = "#00FF41"            
BTN_ACTIVE_BG = "#008F11"     
CONSOLE_BG = "#050505"        
CONSOLE_FG = "#00FF41"        
...
class MatrixScanner(tk.Tk):
    def __init__(self):
        super().__init__()

        self.title("THE MATRIX: ICS & IT SECURITY SCANNER")
        ...
        self.log_file = f"matrix_scanner_activity_{current_time}.log"
    ...
    def create_widgets(self):
        ...
        self.bg_canvas = tk.Canvas(self, width=820, height=720, highlightthickness=0)
        self.bg_canvas.pack(fill="both", expand=True)
        
        try:
            # Resize image to fit window
            img = Image.open("matrix_bg.jpg")
            img = img.resize((820, 720), Image.LANCZOS)
            self.bg_image = ImageTk.PhotoImage(img)
            self.bg_canvas.create_image(0, 0, image=self.bg_image, anchor="nw")
        except Exception as e:
            print(f"No background image found: {e}")
            self.configure(bg=BG_COLOR)
            self.bg_canvas.configure(bg=BG_COLOR)

        # Create a semi-transparent or solid frame over the canvas to hold widgets 
        # (Tkinter doesn't support true widget transparency easily, using a dark generic frame)
        main_frame = tk.Frame(self.bg_canvas, bg=BG_COLOR, bd=5, relief=tk.RIDGE)
        main_frame.place(relx=0.5, rely=0.5, anchor="center")

        title_label = tk.Label(
            main_frame, 
            text="[ ENTER THE MATRIX SCANNER ]", 
            font=FONT_TITLE, 
            bg=BG_COLOR, 
            fg=TEXT_NEON_GREEN
        )
        ...
        self.target_entry = tk.Entry(
            ...
            bg=CONSOLE_BG, 
            fg=TEXT_NEON_GREEN, 
            insertbackground=TEXT_NEON_GREEN,
            width=25,
            relief=tk.FLAT
        )
        ...
        for i, (name, var) in enumerate(self.protocols.items()):
            cb = tk.Checkbutton(
                ...
                activeforeground=TEXT_NEON_GREEN,
                pady=2
            )
    ...
    def start_scan_thread(self):
        ...
        self.scan_btn.config(state=tk.DISABLED, text="[ SCANNING... ]", bg=TEXT_DARK_GREEN)
        self.cancel_btn.config(state=tk.NORMAL)
        self.console.delete(1.0, tk.END)
        
        self.write_log("==================================================")
        self.log_to_console(f"[*] INITIATING MATRIX SCAN ON TARGET: {target}\n")
    ...
    def run_nmap_scan(self, target, selected_ports, selected_scripts):
        ...
        try:
            ...
            if self.cancel_requested:
                ...
                self.after(0, self.log_to_console, "\n[OK] OPERATION COMPLETE. WAKE UP, NEO.\n")
    ...
    def reset_button(self):
        self.scan_btn.config(state=tk.NORMAL, text="INITIATE MATRIX SCAN", bg=BTN_BG)
        self.cancel_btn.config(state=tk.DISABLED)
